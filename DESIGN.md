...
크롤러는 범용 데이터(URL, 제목, 본문, 시각)를 수집하고, 서비스별로 필요한 추가 데이터(ASIN, 영상 URL 등)는 서비스 쪽에서 가공한다. 서비스가 추가돼도 크롤러 코드/포맷은 변경 없고, 탐색 소스만 추가하면 된다.
...
|------|------|--------------|
| **크롤러** | 범용 정보 수집 | 변경 없음 |
| **정보 풀 + API** | 수집 정보 저장, 서비스 연동 | 인터페이스 연결만 |
| **보상 시스템** | 통합 보상 계산/학습 | 서비스별 가중치 설정 |
...
- `sourceUrl` — 필수, 중복 감지용
- `title` — 필수
- `content` — 본문이 풍부할수록 채택률 높음 (300자 이상 권장, 전문 최상)
- `discoveredAt` — 필수, 속도 측정용
- `publishedAt` — 선택, 알 수 있다면 (속도 보상 가중치)
- `type` 없음 — 크롤러는 분류하지 않음
...
| 보상 | 판단 주체 | 방법 |
|------|----------|------|
| 1차 (채택) | 각 서비스 AI | 채택 시 보상 시스템에 알림 |
| 2차 (성과) | 각 서비스 | 성과 데이터를 주기적으로 보상 시스템에 전달 |

보상 시스템은 독립 모듈로, 어떤 서비스에서 온 보상이든 통합 관리.
...
```
submission(수집) → adoption(채택) → page(페이지 생성) → performance(성과)
```
...
GET  /api/crawlers/{id}/stats      크롤러 통계
```

### 6.4 인증
- 크롤러별 API key 발급
- Rate limit: 크롤러당 분당 10~30건
...
### 6.6 서비스 알림
- 새 정보 수집 시 webhook으로 서비스에 실시간 알림
- SEO 선점을 위해 배치보다 즉시 알림이 유리
...
-- 크롤러 등록/관리
crawlers
  id, name, api_key, strategy, status, created_at

-- 수집 정보 저장
submissions
  id, crawler_id, source_url, source_site, title, content,
  discovered_at, published_at, image_urls, language, tags,
  status(pending/reviewed), created_at

-- 채택 기록
adoptions
  id, submission_id, service(review/vmap/...), page_id,
  adopted_at
...
-- 보상 기록
rewards
  id, crawler_id, submission_id, type(adoption/performance),
  service, amount, reason, created_at

-- 크롤러 학습 요약
crawler_stats
  crawler_id, source_site, adoption_rate, avg_reward,
  total_submissions, total_adoptions, last_updated
```

### 7.2 중복 처리
- `submissions.source_url`에 UNIQUE 인덱스
- Bloom Filter로 메모리 레벨 빠른 중복 체크
